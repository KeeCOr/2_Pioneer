...
- 항로 해금 진행 HUD: 상단 HUD에 다음 잠긴 항로 이름, 해금 진행률, 남은 누적 판매 금액을 표시해 초반 장기 목표를 항상 확인할 수 있게 했다.
...
- 지도 레이어 정리: 화물 미니창을 지도 위 오버레이에서 지도 상단의 선택 배 트레이로 이동하고, 줌아웃 상태에서는 항구 라벨·이벤트 라벨·순항 버튼을 줄여 항로와 지형이 먼저 보이게 했다. 목적지 선택 후보선도 가까운 항해 가능 항구 위주로 제한해 선 겹침을 줄였다.
- 패널/버튼 시각선 정리: 공통 카드와 작은 버튼에서는 장식 프레임 배경을 제거하고, 모달 프레임과 내부 구획선의 대비를 낮춰 버튼·창·정보선이 서로 겹쳐 보이는 문제를 완화했다.
...
- 바다 질감 재작업: 지도 바다 배경을 더 밝은 심해 블루 그라데이션, 반복 잔물결, 수면 반짝임, 넓은 해류 광택으로 재구성해 실제 바다처럼 읽히도록 조정했다.
- 항해 화면 밀도 강화: 참고 이미지처럼 항해 중인 배가 흰 돛 선단과 진행도 라벨로 더 잘 보이게 하고, 해류선·점선 무역로·밝은 항적을 추가해 실제로 바다 위를 항해하는 느낌을 강화했다. 대륙 텍스처는 약간 뒤로 물려 바다가 화면의 주인공으로 읽히게 조정했다.
- 정박 수면 좌표 분리: 항구의 도시/라벨 좌표와 배가 정박·출발·도착하는 바다 좌표를 분리했다. 새 게임, 선박 구매, 기존 저장 데이터 표시, 목적지 선택선, 도착 판정 모두 정박 수면 좌표를 사용해 배가 육지 위에 놓여 보이지 않게 했다.
- 항구 화면 고정감 제거: 맵 드래그 중 화면 밖 항구가 가장자리에 붙어 따라오는 것처럼 보이던 문제를 수정했다. 항구 겹침 보정은 현재 화면 근처의 항구에만 적용하고, 화면 밖 항구는 실제 맵 좌표 그대로 밖으로 나가게 했다.
- 핵심 UI 폴리싱: 거래소 팝업에 금화·화물·시세 방향·판매 수수료 요약 스트립을 추가하고, 우측 선택 배 패널에는 선박 엠블럼과 연료·내구·화물 상태판을 먼저 보여주도록 구성했다. 상단 HUD와 항구 라벨도 해양 테마 프레임으로 정리해 정보 위계와 게임 UI 느낌을 강화했다.
- 정박/표시 좌표 재검수: 실제 대륙 텍스처의 투명 바다 픽셀을 기준으로 모든 항구 정박 좌표를 다시 계산했다. 정박 배를 항상 오른쪽으로 54px 밀어내던 고정 오프셋을 제거하고, 단일 정박 배는 바다 정박점에 그대로 표시하며 여러 배가 있을 때만 작은 원형 배치로 분산하도록 수정했다.
- UI/리소스 적용: 필요한 UI 및 리소스 목록을 별도 문서로 정리하고, 금화·보석·거래소·화물·나침반·닻·연료·수리 PNG 아이콘을 새로 제작해 HUD와 거래/항해 버튼에 적용했다. 기존 자원/선박 PNG도 거래소, 화물 인벤토리, 지도 선박, 함대 목록에 연결해 이모지 중심 표현을 줄였다.
- 버튼/프레임 리소스 적용: 패널 프레임, 모달 프레임, 금색 버튼, 해양 버튼, 인벤토리 슬롯 PNG를 제작해 공통 카드, 거래소/시세 팝업, 목적지 확정 버튼, 매입/판매 CTA, 화물 슬롯에 적용했다.
...
- 실제 대륙 이미지 적용: 코드 폴리곤 대륙 레이어를 제거하고, 실제 세계지도 실루엣에 가까운 투명 PNG 대륙 텍스처를 생성해 지도 배경에 적용했다. 대륙 이미지는 지도 pan/zoom과 같은 좌표계로 움직인다.
- 항구 좌표 재정렬: 실제 대륙 이미지 기준으로 유럽·지중해·아라비아·인도·동남아·동아시아·아메리카 항구 위치를 다시 배치하고, 시작 카메라를 1.35배 줌인 상태로 조정했다.
...
- 플레이 예시 이미지 최신화: 정리된 지도 레이어와 선택 배 트레이 구조에 맞춰 `Pioneer_gameplay_preview.png`를 고품질 1920×1080 렌더 이미지로 교체했다.
...
- 자연 학습형 튜토리얼: 상단 항해사 조언에 학습 목표와 단계 진행도를 추가하고, 배 선택 → 항구 시세 확인 → 목적지 확정 → 항해 관찰 → 판매 → 매입 → 재출항 흐름을 실제 플레이 행동과 연결했다.
- 목적지 확정 학습: 항로 선택 중 항구를 누르면 바로 출항하지 않고 시세창을 먼저 보여주며, 튜토리얼 단계에서는 [목적지 확정] 버튼과 추천 항구가 강조된다.
- 튜토리얼 UI 안정화: 조언 바 높이를 늘려 목표 문구와 진행 바가 겹치지 않게 하고, 튜토리얼을 숨기더라도 기존 상시 조언은 계속 동작하도록 유지했다.
...
- 이미지/스프라이트: `src/assets/frames/modal-nautical.png`, `src/assets/map/world-landmasses.png`

메모:
- 리소스 후보 57개 중 자동 참조 확인 2개.
...
![2_Pioneer world-landmasses](../src/assets/map/world-landmasses.png)
- `src/assets/map/world-landmasses.png`

![2_Pioneer modal-nautical](../src/assets/frames/modal-nautical.png)
- `src/assets/frames/modal-nautical.png`

<!-- RESOURCE_PREVIEWS_END -->


## 2026-06-19 업데이트 - SVG 런타임 이미지 교체
- 런타임 UI/아이콘/프레임 SVG 파일을 PNG 자산으로 교체했다.
- 앱 로딩 경로를 PNG glob으로 갱신하고, 모달 프레임 CSS 참조를 PNG로 변경했다.
- 차트와 항로 지도에 쓰이는 인라인 SVG는 데이터 시각화 레이어로 남겨두고 별도 개선 대상으로 분리했다.

## 2026-06-19 업데이트 - 인라인 SVG 제거
- 가격 그래프와 항로/해류/격자 지도 레이어를 Canvas 렌더링으로 전환했다.
- 런타임 앱 코드의 SVG 이미지 사용을 제거하고 PNG 자산 및 Canvas 기반 시각화로 정리했다.
